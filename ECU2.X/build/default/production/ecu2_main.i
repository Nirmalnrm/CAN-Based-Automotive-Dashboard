...
uint16_t get_RPM() {
    ...
    read_adc(0x04);
    uint16_t adc_reg_val = read_adc(0x04);
    return (1500 + ((uint32_t)adc_reg_val *5500) / 1023);
}
...
IndicatorStatus process_indicator()
{
    IndicatorStatus cur_ind_status = prev;
    unsigned char key = read_switch(1);
